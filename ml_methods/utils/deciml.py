...
def deciml(__a:float|int|str|Decimal,__pr=None)->Decimal:
    try:
        global __DecimalPrecision
        __pr=__DecimalPrecision
...
class constant:
    ...
    def e(pr=getpr())->Decimal|None:
        ...
        except:retrn('c',"Invalid argument: pr -> < 100");
...
def abs(__a:float|int|str|Decimal)->Decimal:
    a=Decimal(str(__a))
...
class stat:
    ...
    def median(x:list[Decimal]|tuple[Decimal,...])->Decimal:
        try:
            lm=algbra.div(len(x),2);(x:=list(x)).sort();
            if (i:=int(lm)-1)==lm:return x[i];
            else:return(algbra.div(algbra.add(x[i],x[i+1]),2));
        except:return Decimal('NaN');
    ...
    def mode(x:list|tuple)->Decimal|tuple[Decimal,...]:
        try:
            d=dict();r=list();
            for i in x:d[i]=d.setdefault(i,0)+1;
            c=max(d.values())
            for i in d.items():
